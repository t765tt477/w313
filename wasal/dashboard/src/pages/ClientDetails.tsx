...
export default function ClientDetails() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'drivers' | 'orders' | 'users' | 'analytics' | 'settings' | 'admin-management' | 'logs' | 'cities'>('users');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'فشل حذف الزبون');
    }
  };
  ...
  if (!client) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-red-600 font-semibold">الزبون غير موجود</div>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout activeTab={activeTab} onTabChange={setActiveTab} onRefresh={handleRefresh} refreshing={refreshing}>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-black text-green-800">بيانات الزبون</h1>
